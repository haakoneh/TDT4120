...
def antallIsolerteStier(kapasiteter, startrom, utganger):
	...
	nodes = [[0 for i in range(n * 2 + 2)] for j in range(n * 2 + 2)]
    
	for i in range(len(startrom)):
		nodes[0][startrom[i] * 2 + 1] = 1
        
	for i in range(len(utganger)):
		nodes[utganger[i] * 2 + 2][-1] = 1
	...
	t = finnFlytsti(0, n * 2 + 1, temp, nodes)
